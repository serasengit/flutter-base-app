...
- Flutter for UI, platform integration and app runtime
- Dart for the application language

Key libraries currently used in the template:

- `flutter_bloc` and `bloc` for auth state and shell/module state management
...
- `get_it` for dependency registration and lookup
- `logger` for application logging
- `package_info_plus` for app version and build metadata shown in the UI

Keep this section aligned with `pubspec.yaml` when the stack changes.
